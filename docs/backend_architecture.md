...
- **A search waits for the load and never performs it.** `query_encoders()` hands out the copies only once `CpuQueryEncoders.is_loaded()`; before that it queues the load if none is pending and waits on the load task, up to `Vault.CPU_QUERY_ENCODER_LOAD_WAIT_S` (60 s). A load that does not finish in time, fails, was cancelled (`TaskRunner.cancel_pending_tasks`, which a full restore calls) or cannot be queued (runner stopped) raises `CpuQueryEncodersNotReadyError`; `GET /pictures/search` and `POST /pictures/likeness-search` turn it into 503 ("Search is still loading its models"), and the next search queues a failed or cancelled load again. A load already pending or finished is shared. With no running GPU worker (`TaskRunner.is_gpu_worker_alive()` false) the search raises at once without queueing, a waiting search polls liveness every `TaskRunner.GPU_CALL_LIVENESS_POLL_S`, and a call on the GPU worker itself loads the copies inline. `Vault._build_engine` (both engine-build sites) queues the load before assigning `_engine`, so a finder cannot queue a GPU batch ahead of it.
- **They stay resident.** Nothing unloads them: `InferenceEngine.close` and the idle unload name only the engine's own services, so the copies (0.2–0.6 GB) live as long as the engine, and a search never lazily reloads a model on its own thread. `Vault.stop` drops its handle on the load with the engine.

Measurements (latency, ranking parity with Metal-computed vectors, crash runs) are in `docs/apple-metal-thread-safety.md`.

**A missed path raises instead of crashing.** `TaskRunner.start()` registers its GPU worker with `utils/device_utils.register_metal_thread`. The registration is cleared (only if it is still the registered thread) by `stop()` once the worker has exited, or by the worker as it leaves its loop; a worker still running after `STOP_JOIN_TIMEOUT_S` stays registered. `ensure_metal_thread(device)` raises `RuntimeError`, naming the calling thread and pointing at `Vault.run_inference`, when `device` is `mps`, a worker is registered, and the caller is not it; it is silent on other devices and with no running runner, and a registered worker that died of an uncaught exception still refuses every other thread, saying so. While a registered worker runs on Metal, `Vault.stop()` leaves the engine and the class-level models loaded (the engine in `Vault._engines_left_loaded`, with that worker) and still closes the database; the next `Vault.start()` queues each such engine's close on its own GPU worker once the recorded worker has exited, dropping the entry only when the close has run, and `Vault.start()` with a Metal engine waits up to `PREVIOUS_METAL_WORKER_WAIT_S` for it before refusing. A failed `Vault.start()` stops what it started and closes the vault before re-raising. It sits at the entry points non-task code reaches, not in inner loops: `SBertService.encode`, `ClipService.encode_text` / `encode_image_batch` / `encode_image_crops`, `ModelLifecycleManager.aggressive_unload` / `safe_idle_unload` (before anything is unloaded) and `PixlStashTaggerService.localize_anomaly`. A new route that runs a model on the engine's device goes through `Vault.run_inference`; the guard is the backstop, not the mechanism.
...
All torch models run on CUDA, Apple Metal (MPS) or CPU, chosen by `utils/device_utils.detect_device()`. Two Metal-specific routes are deliberate, not oversights. **Florence-2 loads straight onto Metal in fp16**, as it does on CUDA, and a failed Metal load records a fallback and loads fp32 on the CPU. A direct load through `transformers`' threaded loader crashes, because its threads cast weights on Metal at the same time (`docs/apple-metal-thread-safety.md`); `configure_metal_model_loading()` keeps that loader on one thread, and fp16 is the checkpoint's own dtype. Its Metal batch is capped by memory, not the CUDA constant: `Florence2Service._metal_batch_cap()` fits `metal_base_mb + metal_per_image_mb * batch` (per variant, in `FLORENCE_MODEL_VARIANTS`) into `FLORENCE_METAL_MEMORY_FRACTION` (half) of `torch.mps.recommended_max_memory()`, read once at construction. Measured on a 32 GB M1 Pro (24.96 GiB working set), one 32-image caption call peaked at 8.2 GiB for base (batch 32) and 10.8 GiB for large-ft (batch 20), against 13.1–13.9 and 25.9–26.2 GiB in fp32 at batch 32, with 30–31 of 32 captions identical to fp32 and 168 of 170 fp32 detection boxes matched (IoU ≥ 0.9). WD14 and the built-in tagger batch at most 8 images on Metal (`TaggingWorkflow._MAX_CONCURRENT_METAL`), not the GPU size of 64: no VRAM budget bounds a Metal batch and it all comes out of the machine's one pool of memory. The limit is the taggers' alone; `InferenceEngine.max_concurrent_images`, which Florence-2 sizes from, is unchanged. And the **ONNX** models take their own route — **WD14 through `CoreMLExecutionProvider`**, with `CPUExecutionProvider` listed after it (onnxruntime runs the operators CoreML declines on the CPU either way; listing it keeps the node-assignment warnings out of the log). The CoreML session is created with `enable_fallback=0`, because onnxruntime's default rebuilds a failed session on the CPU and reports the error only with `print()`: a CoreML failure, at start or as an `EPFail` during a run, is logged at WARNING with the model path, the provider options and the error, WD14 then creates a CPU session explicitly (a failed batch is retried on it once), and that service stays on the CPU across reloads. Other run errors fail the batch and leave the session on CoreML. Every other accelerator session, WD14's on CUDA, ROCm or OpenVINO and InsightFace's on CUDA, keeps onnxruntime's default fallback, so WD14 and InsightFace read `get_providers()` after the session loads and after each batch, and log a WARNING, once per loaded session, when onnxruntime has put it on the CPU. CoreML compiles the model on every load (11-13 s against 0.5 s on the CPU with the real checkpoint); the trade-off against a `ModelCacheDirectory` is recorded on `_COREML_PROVIDERS`. Meanwhile **InsightFace stays on the CPU provider** because CoreML compiles a static shape against its SCRFD detector's dynamic `[1, 3, ?, ?]` input and then fails at the `det_size` the face pipeline configures.
